...
bool PbClientModule::Initialize(aimrt::CoreRef core) {
  ...
  try {
    ...
    #if USING_SYNC
    // 创建一个同步型 proxy
    sync_proxy_ = std::make_shared<example::protocols::test::InfoRpcSyncProxy>(rpc_handle_);
    #elif USING_ASYNC 
    // 创建一个异步回调型 proxy
    async_proxy_ = std::make_shared<example::protocols::test::InfoRpcAsyncProxy>(rpc_handle_);
    #elif USING_FUTURE
    // 创建一个异步future型 proxy
    future_proxy_ = std::make_shared<example::protocols::test::InfoRpcFutureProxy>(rpc_handle_);
    #elif USING_CO
    // 创建一个无栈协程型 proxy
    co_proxy_ = std::make_shared<example::protocols::test::InfoRpcCoProxy>(rpc_handle_);
    #endif
